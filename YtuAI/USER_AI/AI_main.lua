...
--[[
	Initial setup function. This is called the first time the AI runs.
	It checks the user's configuration for conflicting settings, loads saved data,
...
--[[
	Handles the MOVE_CMD command.
	This is called when the user right-clicks on the ground to move.
--]]
...
--[[
	Handles the STOP_CMD command.
	This is called when the user uses the /hvan_stop command.
...
--[[
	Handles the ATTACK_AREA_CMD command.
	This is called when the user shift-right-clicks on the ground to attack an area.
	In practice, this command is never sent by the client, so this is legacy code.
--]]
...
--[[
	Handles the PATROL_CMD command.
	This is called when the user shift-right-clicks on the ground to patrol an area.
	In practice, this command is never sent by the client, so this is legacy code.
--]]
...
--[[
	Handles the HOLD_CMD command.
	This is called when the user uses the /hvan_hold command.
	In practice, this command is never sent by the client, so this is legacy code.
--]]
function	OnHOLD_CMD ()
	TraceAI ("OnHOLD_CMD")
...
--[[
	Handles the SKILL_OBJECT_CMD command.
...
--[[
	Handles the SKILL_AREA_CMD command.
	This is called when the user uses a skill on the ground.
--]]
...
--[[
	Handles the FOLLOW_CMD command.
	This is called when the user uses the /hvan_follow command.
	It toggles between following the owner and being idle.
--]]
...
--[[
	...
	This is the main state where the AI decides what to do next.
...
--[[
	Handles the FOLLOW_ST state.
	This is called when the homunculus needs to move to the owner's location.
--]]
...
--[[
	Handles the CHASE_ST state.
	This is called when the homunculus is chasing a monster.
--]]
function	OnCHASE_ST ()
...
function OnATTACK_ST ()
	...
	-- Dynamic Kiting System: Check for reactive kiting
	local should_kite = false
	if K_ReactiveKiteHP > 0 and HPPercent(MyID) < K_ReactiveKiteHP then
		TraceAI("Reactive Kiting: HP is low, starting to kite.")
		should_kite = true
	end

	if ((UseSkillOnly ~= 1 and (UseDanceAttack==1 or should_kite) and GetV(V_SP,MyID) >= DanceMinSP) or (BerserkMode==1 and Berserk_Dance==1) or (panicmode==1 and Panic_UseDanceAttack==1 and HPPercent(MyID) > FleeHP)) and (IsHomun(MyID)==1 and MySkill==0) and GetDistanceRect(MyEnemy,GetV(V_OWNER,MyID)) < 13 then
			local alt_dance = 0
			-- Stuck detection for kiting
			if K_StuckDetection == 1 and MyStates[1] == ATTACK_ST and MyStates[2] == ATTACK_ST and MyPosX[1] == MyPosX[3] and MyPosY[1] == MyPosY[3] then
				TraceAI("Kiting Stuck Detection: Trying alternative dance cell.")
				alt_dance = 1
			end

			nx,ny=GetDanceCell(MyAttackStanceX,MyAttackStanceY,MyEnemy,alt_dance)
...
function AI(myid)
	...
	if OriginalSummonTick then
		...
		if EnableSuicideTactic == 1 and elapsedTime > timerThreshold and MyState ~= SUICIDE_ST then
			...
			-- logappend("AAI_ERROR", "ERROR: FORCING SUICIDE STATE - elapsed time: " .. math.floor(elapsedTime/60000) .. " minutes exceeds threshold: " .. SuicideTimer)
